...
<div class="card">
    <div class="card-body">
        <div class="d-flex align-content-center justify-content-between mb-3">
            <h4 class="card-title mb-0">Data Portofolio</h4>
            <a href="/portofolio/create" class="btn btn-primary">
        ...
        <table class="table table-striped table-bordered">
            <thead>
              <tr>
                <th scope="col">#</th>
                <th scope="col">
    <a href="?sort_name=title&sort_val={{$sort}}">title
        @if (request('sort_name')=='title' && request('sort_val')=='ASC')
...
<td>{{$item->gambar}}</td>
                    ...
                    <td>
                        <a class="btn btn-warning btn-sm" href="/portofolio/{{$item->id}}">
                            <span data-feather="eye"></span>
                        </a>
                        <a class="btn btn-primary btn-sm" href="/portofolio/{{$item->id}}/edit">
                            <span data-feather="edit"></span>
                        </a>

                        <a class="btn btn-danger btn-sm" href="javascript:" onclick="deleteData({{$item->id}})"><span data-feather="trash"></span></a>
                        <form class="d-none" id="formdelete-{{$item->id}}" action="/portofolio/{{$item->id}}" method="POST">
                            @method('DELETE')
                            @csrf
                        </form>
                    </td>
